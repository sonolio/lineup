...
# saves these discrepencies into json for future use
def lineup_to_ids(lineup, js, csv):
    if lineup is None:
        return None
    id_lineup = [None] * 8
...
def generate_csv(lineups):
    ...
    for i in range(len(lineups)):
        ...
        a = lineup_to_ids(lineup_to_positions(lineups[i], positions=[None]*8), js, csv)
        if a is not None:
            df.loc[i] = a
            j += 1
